...
class QdrantMemory:
    ...
    async def ensure_collections(self) -> None:
        ...
        try:
            existing = {
                collection.name for collection in (await client.get_collections()).collections
            }
    ...
    async def upsert(self, collection: str, point: MemoryPoint) -> None:
        ...
        client = self._client()
        try:
            vector = await self._embedder.embed(point.text)
            payload = {
                **point.payload,
                "text": point.text,
            ...
            await client.upsert(
                collection, [models.PointStruct(id=point.point_id, vector=vector, payload=payload)]
            )
        finally:
            await client.close()
